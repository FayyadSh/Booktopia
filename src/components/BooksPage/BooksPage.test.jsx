...
jest.mock("../Pagination/Pagination", () => ({
  __esModule: true,
  default: ({ dataArray, length }) => (
    <div data-testid="pagination">{`Showing ${dataArray.length} of ${length}`}</div>
  ),
}));
...
describe("BooksPage Component", () => {
  ...
  it("renders the NoContent component when there are no books and no error", () => {
    useGlobalContext.mockReturnValue({ error: null });
    render(<BooksPage pageName="Test Page" pageBooks={{ books: [], total: 0 }} />);

    expect(screen.getByTestId("no-content")).toHaveTextContent("No Content");
  });

  it("does not render Pagination when books array is empty", () => {
    useGlobalContext.mockReturnValue({ error: null });
    render(<BooksPage pageName="Test Page" pageBooks={{ books: [], total: 0 }} />);
